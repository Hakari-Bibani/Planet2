...
def handle_payments(entry_type):
    st.markdown("<h2 style='color: #2c3e50; text-align: center;'>Payment Management</h2>", unsafe_allow_html=True)
    
    if entry_type == "Modify/Delete":
        ...
        if data:
            ...
            with action_tabs[0]:
                ...
                if st.button("Update Payment"):
                    update_query = """
                    UPDATE payments SET tree_name=%s, customer_full_name=%s, username=%s, quantity=%s, amount=%s, address=%s, whatsapp_number=%s, email=%s, payment_preference=%s, payment_date=%s, status=%s, note=%s
                    WHERE payment_id=%s;
                    """
...
def main():
    st.markdown("<h1 style='text-align: center; color: #2c3e50;'>Payment Management System</h1>", unsafe_allow_html=True)
    handle_payments("Modify/Delete")
